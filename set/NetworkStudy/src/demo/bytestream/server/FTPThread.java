...
public class FTPThread extends Thread {
	...
	private void sendFileNames() throws IOException{
		...
		StringBuilder sb = new StringBuilder(); // Stringbuffer랑 같은 역할인데 동기화 안함 
		for (File file : list) {
			String name = file.getName();
			sb.append(name + "("+file.length()+"byte),");
		}
		out.writeUTF(sb.toString()); // 텍스트를 보여주기 위해 write utf 써줌
	...
	public void sendFile(String fileName) throws IOException{
		...
		out.writeLong(fileSize); // 파일 받기 전에 미리 보냄 : 상태 출력할 때 계산하려고!
