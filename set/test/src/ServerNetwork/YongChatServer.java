...
public class YongChatServer {
   ...
   //현재인원 최대인원 비교
   public boolean compareRoomCnt(int roomNumber) {
      int cnt = participate.get(roomNumber).size();
//      System.out.println("현재인원:::"+ cnt);
      Enumeration<YongClient> client = participate.get(roomNumber).elements();
      Enumeration<String> leader = rooms.keys();
      String cap = "";
      while (client.hasMoreElements()) {
         YongClient yongClient = (YongClient) client.nextElement();
         while (leader.hasMoreElements()) {
            String name = (String) leader.nextElement();
            if (name.equals(yongClient.getNickName())) {
               cap = name;
            }
         }
      }
      
      YongRoom room = rooms.get(cap);
//      System.out.println("최대인원:::::"+room.getMaxRoomCnt());
      room.getMaxRoomCnt();
      if(Integer.parseInt(room.getMaxRoomCnt()) == cnt) {
         return false;
      }else {
         return true;
      }
   ...
   public boolean isBack(int roomNumber, String nickName, int protocol) {
      ...
      if (rooms.containsKey(nickName)) { // 방장이 나갈 시
         ...
         while (client1.hasMoreElements()) {
            ...
            if (!yongClient.getNickName().equals(nickName)) { // 방장 아닌 사람들은 대기실로
         ...
         if (protocol != YongProtocol.DISCONNECT) { // DISCONNECT가 아니면 대기실로 다시 돌려보내줌
   ...
   public String participateAll(int roomNumber, String message) {
      ...
      Enumeration<YongClient> e = participate.get(roomNumber).elements(); // 방번호에 해당하는 방에 있는 클라이언트들
      while (e.hasMoreElements()) {
         YongClient yongClient = (YongClient) e.nextElement();
   ...
   public int removeClient(String nickName) {
      ...
      if (clients.containsKey(nickName)) { // 삭제하고자 하는 대상이 대기실에 있을 경우
         ...
         for (int i = 0; i < participate.size(); i++) { // 삭제하고자 하는 대상이 방안에 있을 경우
            Enumeration<YongClient> client = participate.get(i).elements();
            while (client.hasMoreElements()) {
               YongClient yongClient = (YongClient) client.nextElement();
               if (yongClient.getNickName().equals(nickName)) {
