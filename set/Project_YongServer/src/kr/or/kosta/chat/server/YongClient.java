...
import kr.or.kosta.chat.common.YongProtocol;
   ...
   public void process(String message) {
      ...
      case YongProtocol.CONNECT:
         ...
         if(chatServer.isExistNickName(nickName)) {
            sendMessage(YongProtocol.CONNECT_RESULT + YongProtocol.DELEMETER + nickName + YongProtocol.DELEMETER +  "FAIL");
            
         }else {
            time = Long.parseLong(tokens[2]);
            chatServer.addClient(this);
            System.out.println("[Debug] : 접속 클라이언트 수 : " + chatServer.getClientCount());
            chatServer.sendAllMessage(YongProtocol.CONNECT_RESULT + YongProtocol.DELEMETER + nickName + YongProtocol.DELEMETER +  "SUCCESS");
         }
         break;
