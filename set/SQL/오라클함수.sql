...
--WHERE LOWER(first_name) = 'james'; -- 행 자체를 소문자로 바꿔서 
WHERE UPPER(first_name) = UPPER('james'); -- 행 자체를 대문자로 바꿔서
...
SELECT SUBSTR('Java Developer', 6, 9)-- 6부터 9개 불러오기 : 1부터 시작
FROM dual;

SELECT SUBSTR('서울시가산동', 4)
FROM dual; --몇개가져오는지 언급 없으면 다 가져옴
...
FROM dual; --a 1번째로 찾은 거에서 두번째 지난 자릴 수 
